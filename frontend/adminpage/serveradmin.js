...
function apiFetch(path, opts) {
    ...
    return fetch(url, newOpts).then(async res => {
        if (!res.ok) {
            try {
                // Clone the response to read the body without consuming the original stream
                const errData = await res.clone().json();
                if (errData.error_id) {
                    console.group(`%c Error ID ${errData.error_id}`, 'color: white; background: red; padding: 2px 5px; border-radius: 2px;');
                    console.log(`%c Message: ${errData.message}`, 'font-weight: bold; color: #ff4444;');
                    console.log(`%c Timestamp: ${errData.timestamp}`, 'color: gray;');
                    console.log(`%c Context:`, 'color: #007bff;', errData.context);
                    console.groupEnd();
                } else if (errData.error_code) {
                    // Fallback for old error code if present
                    console.group(`%c Error ${errData.error_code}`, 'color: white; background: red; padding: 2px 5px; border-radius: 2px;');
                    console.log(`%c Message: ${errData.message}`, 'font-weight: bold; color: #ff4444;');
                    console.groupEnd();
                }
            } catch (e) {
...
document.addEventListener('DOMContentLoaded', async function() {
            ...
            try {
                ...
                if (cameraList) {
                    if (err.message.includes("HTTPS")) {
                        cameraList.innerHTML = '<p class="error-message" style="color:red">Camera access blocked. Use <b>localhost</b> or <b>HTTPS</b>.</p>';
                    } else {
                        cameraList.innerHTML = '<p class="error-message" style="color:red">Error accessing cameras. Check permissions.</p>';
                    }
                }
            }
        });
    ...
    if (markManualBtn) {
        markManualBtn.addEventListener('click', async () => {
            const roll_number = manualStudent.value;
            if (!roll_number) {
        ...
        try {
            ...
            if (video) {
                if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
                    throw new Error("Camera API not available. Browsers require HTTPS or localhost for camera access.");
                }
            ...
            alert(`Could not start camera: ${err.message}\n\nNote: If you are accessing this via an IP address (e.g., 192.168.x.x), browsers block camera access for security. Please use 'localhost' or set up HTTPS.`);
        }
    }
    ...
    if(submitRegBtn) {
        submitRegBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                if (response.ok) {
                    alert(`Student Registered Successfully!\nroll_number: ${res.roll_number || 'Generated'}`);
    ...
    if (startAttendanceBtn) {
        startAttendanceBtn.addEventListener('click', async () => {
            ...
            try {
                if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
                    throw new Error("Camera API not available. Browsers require HTTPS or localhost.");
                }
        ...
        dynamicContent.innerHTML = `
            ...
            <table style="width:100%; border-collapse:collapse; background:white; border-radius:12px; overflow:hidden;">
                ...
                <tbody>
                    ${students.map(s => `
                        <tr style="border-bottom:1px solid #eee;">
                            <td style="padding:15px;">${s.roll_number}</td>
                            <td style="padding:15px;"><input type="text" value="${s.name}" id="name-${s.roll_number}" style="padding:5px; border:1px solid #ddd; border-radius:4px;"></td>
                            <td style="padding:15px;"><input type="text" value="${s.course}" id="course-${s.roll_number}" style="padding:5px; border:1px solid #ddd; border-radius:4px;"></td>
                            <td style="padding:15px;">
                                <button onclick="updateStudent('${s.roll_number}')" style="background:var(--success); color:white; border:none; padding:5px 10px; border-radius:4px; cursor:pointer;">Save</button>
                                <button onclick="deleteStudent('${s.roll_number}')" style="background:var(--danger); color:white; border:none; padding:5px 10px; border-radius:4px; cursor:pointer; margin-left:5px;">Delete</button>
                            </td>
                        </tr>
                    `).join('')}
                </tbody>
